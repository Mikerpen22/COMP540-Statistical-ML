...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            dist_to_train = self.X_train - X[i]
            dist_to_train = np.linalg.norm(dist_to_train, axis=1)
            dists[i] = dist_to_train
            pass
            #######################################################################
            #                         END OF YOUR CODE                            #
            #######################################################################
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            labelsCnt = cl.Counter(closest_y)
            agree_label = -1
            agree_cnt = 0
            for label in labelsCnt.items():
                if label[1] > agree_cnt:
                    agree_cnt = label[1]
                    agree_label = label[0]
                elif label[1] == agree_cnt:
                    if label[0] < agree_label:
                        agree_label = label[0]
                        agree_cnt = label[1]

                y_pred[i] = agree_label
