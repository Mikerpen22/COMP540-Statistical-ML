...
def get_CIFAR10_data(num_training=49000, num_validation=1000, num_test=10000):
  ...
  # subsample the data
    X_train, y_train, X_val, y_val, X_test, y_test = subsample(
        num_training, num_validation, num_test, X_train, y_train, X_test, y_test)
...
def load_CIFAR_batch(filename):
    ...
    with open(filename, 'rb') as f:
        ...
        X = X.reshape(10000, 3, 32, 32).transpose(0, 2, 3, 1).astype(
            "float")  # interesting usage of transpose!
        Y = np.array(Y)
        return X, Y
...
def preprocess(X_train, X_val, X_test):
    ...
    # Preprocessing: reshape the image data into rows (flatten 32*32*3 -> 3073)
