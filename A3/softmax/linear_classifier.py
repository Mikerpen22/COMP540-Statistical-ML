...
class LinearClassifier:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # choose batch_size numbers from range(0, num_train)
    ...
    def predict(self, X):
        ...
        ###########################################################################
        pred = np.dot(X, self.theta)  # (m, K)
        pred_max = np.max(pred, axis=1).reshape((-1, 1))  # (m, 1)
        pred -= pred_max
        pred = np.exp(pred)
        pred_sum = np.sum(pred, axis=1).reshape((-1, 1))
        pred /= pred_sum  # (m, K)
