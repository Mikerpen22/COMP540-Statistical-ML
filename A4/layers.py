...
def affine_forward(x, theta, theta0):
    ...
    #############################################################################
    m, d = x.shape[0], 1
    for i, d_i in enumerate(x.shape):
        if i != 0:
            d *= d_i
    # x will be stored in cache so don't overwrite x itself
    x_flat = x.reshape((m, d))
    out = np.matmul(x_flat, theta) + theta0  # (m,d) * (d,h) -> (m, h)
...
def conv_forward_naive(x, theta, theta0, conv_param):
    ...
    output_shape = (m, F, 1 + int((H + 2 * pad - HH) / stride),
                    1 + int((W + 2 * pad - WW) / stride))
    out = np.zeros(output_shape)

    # pad_for_axes[dimension] = (padding at front, padding at end)
    pad_for_axes = [(0, 0), (0, 0), (pad, pad), (pad, pad)]
    x_pad = np.pad(x, pad_for_axes, 'constant', constant_values=0)

    # Fill in the output tensor out
    for i in range(m):    # Loop through inputs
        for j in range(F):  # Loop through different filters
            # Loop through height starting point
            for k in range(output_shape[2]):
                # loop through width starting point
                for p in range(output_shape[3]):
                    x_pad_slice = x_pad[i, :, k*stride:k *
                                        stride+HH, p*stride:p*stride+WW]
                    out[i, j, k, p] = np.sum(
                        x_pad_slice * theta[j]) + theta0[j]
...
def conv_backward_naive(dout, cache):
    ...
    #     for _f in range(F):
    #         for _c in range(C):
    #             # dx = full convolution(filter rotated 180, upsteam derivative)
    #             # dx[_m, _c, :, :] += signal.convolve2d(
    #             #     dout[_m, _f, :, :], theta[_f, _c, :, :], mode='full', boundary='fill', fillvalue=0)
    #             dtheta[_f, _c, :, :] += signal.convolve2d(
    #                 x_pad[_m, _c, :, :], np.flipud(np.fliplr(dout[_m, _f, :, :])), mode='valid', boundary='fill', fillvalue=0)

    for _m in range(m):
        for _f in range(F):
            for _h in range(1 + (H + 2 * pad - HH) // stride):
                for _w in range(1 + (W + 2 * pad - WW) // stride):
                    dx[_m, :, _h*stride:_h*stride+HH, _w*stride:_w *
                        stride+WW] += dout[_m, _f, _h, _w]*theta[_f]
                    dtheta[_f, :, :, :] += dout[_m, _f, _h, _w]*x_pad[_m,
                                                                      :, _h*stride:_h*stride+HH, _w*stride:_w*stride+WW]
        ...
        for _c in range(C):
            for _h in range(out_H):
                for _w in range(out_W):
                    out[_m, _c, _h, _w] = np.max(
                        x[_m, _c, _h*stride:_h*stride+pool_height, _w*stride:_w*stride+pool_width])
...
def max_pool_backward_naive(dout, cache):
    ...
    for _m in range(m):
        for _c in range(C):
            for _h in range(out_H):
                for _w in range(out_W):
                    mat = x[_m, _c, _h*stride:_h*stride +
                            pool_height, _w*stride:_w*stride+pool_width]
                    # Extract which local position this max element is at -> use to locate the actual position
                    max_elem_coord = np.unravel_index(mat.argmax(), mat.shape)
                    h_shift, w_shift = max_elem_coord[0], max_elem_coord[1]
                    dx[_m, _c, _h*stride+h_shift, _w *
                        stride+w_shift] = dout[_m, _c, _h, _w]
                    #############################################################################
                    #                             END OF YOUR CODE                              #
                    #############################################################################
    return dx
