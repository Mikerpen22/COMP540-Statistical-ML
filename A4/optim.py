...
def rmsprop(theta, dtheta, config=None):
    ...
    config["cache"] = config["decay_rate"]*config["cache"] + \
        (1-config["decay_rate"]) * (dtheta*dtheta)
...
def adam(theta, dtheta, config=None):
    ...
    # stored in config.                                                         #
    #############################################################################
    # Update momentum(1st moment) & second moment & time t
    config["m"] = config["beta1"]*config["m"] + (1-config["beta1"])*dtheta
    config["v"] = config["beta2"]*config["v"] + \
        (1-config["beta2"])*dtheta*dtheta
    config["t"] += 1

    # Correct biased first/second moment
    ...
    next_theta = theta - config["learning_rate"] * \
        m_bias/(np.sqrt(v_bias) + config["epsilon"])
